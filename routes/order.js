  ...
  db.getDB()
    ...
    .toArray((err, doc) => {
      if (err) {
        ...
        if (doc.length == 0) res.status(404).jsonp('No order placed yet!');
        else res.status(200).jsonp(doc);
      }
    });
});

router.get('/api/order/:emailId', midWare.checkToken, (req, res, next) => {
    ...
    dataCollect.updateOne(
      ...
      (iErr, result) => {
        if (iErr) {
          res.status(410).jsonp(iErr);
          next(iErr);
        } else res.status(201).jsonp('Order updated successfully!');
      }
    );
  }
});
