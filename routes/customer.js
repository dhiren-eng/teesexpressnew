        ...
        collection.countDocuments({logName: req.body.usrEmail}, (err, doc) => {
            if(err) {
                ...
                if(doc == 0) {
                    ...
                    res.status(410).jsonp("Your email address already registered... Try different one!");
                }
            }
        });
    }
...
router.post('/api/login', (req, res, next) => {
    if(!req.body.usrName || !req.body.yrPass) {
        ...
        db.getDB().collection('customer').findOne({logName: req.body.usrName}).then((doc) => {
            if (doc) {
                if(doc.accStatus) {
                    if(bcrypt.compareSync(req.body.yrPass, doc.passCode)) {
                        let token = jwt.sign({cId: doc._id, cName: doc.usrName, cEmail: doc.logName}, config.secret);
                        res.status(202).jsonp({cId: doc._id, cName: doc.usrName, token: token});
                    } else
